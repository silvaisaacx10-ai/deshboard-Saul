/* ==========================================================================
   AVENIDA SAUL ELKIND RETAIL DASHBOARD - DATA ENGINE (COM SUPORTE A FILTROS)
   ========================================================================== */
...
const SaulElkindData = {
    ...
    // Função de filtro dinâmico de período para a demanda
    getDemandaByPeriod(period) {
        if (period === 'semestre1') {
            return {
                meses: this.demanda.meses.slice(0, 6),
                vendaReal: this.demanda.vendaReal.slice(0, 6),
                previsaoIA: this.demanda.previsaoIA.slice(0, 6),
                eventos: this.demanda.eventos.slice(0, 6)
            };
        } else if (period === 'semestre2') {
            return {
                meses: this.demanda.meses.slice(6, 12),
                vendaReal: this.demanda.vendaReal.slice(6, 12),
                previsaoIA: this.demanda.previsaoIA.slice(6, 12),
                eventos: this.demanda.eventos.slice(6, 12)
            };
        } else if (period === 'picos') {
            // Apenas meses de picos (Março, Maio, Junho, Julho, Novembro, Dezembro)
            const indices = [2, 4, 5, 6, 10, 11];
            return {
                meses: indices.map(i => this.demanda.meses[i]),
                vendaReal: indices.map(i => this.demanda.vendaReal[i]),
                previsaoIA: indices.map(i => this.demanda.previsaoIA[i]),
                eventos: indices.map(i => this.demanda.eventos[i])
            };
        }
        return this.demanda;
    }

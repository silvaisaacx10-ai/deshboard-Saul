/* ==========================================================================
   AVENIDA SAUL ELKIND RETAIL DASHBOARD - APP CONTROLLER (V2)
   ========================================================================== */
...
function initTheme() {
    ...
    const savedTheme = localStorage.getItem('saul_elkind_theme') || 'dark';
    ...
    themeBtn?.addEventListener('click', () => {
        ...
        localStorage.setItem('saul_elkind_theme', newTheme);
...
// NAVIGATION TABS WITH KEYBOARD ACCESSIBILITY (A11Y)
function initTabs() {
    const tabs = document.querySelectorAll('.nav-tab');
    tabs.forEach((tab, index) => {
        tab.addEventListener('click', () => switchTab(tab));

        // Keyboard navigation (Arrow Left, Arrow Right, Home, End)
...
function switchTab(activeTab) {
    ...
    const targetPaneId = activeTab.getAttribute('data-tab');
    document.querySelectorAll('.tab-pane').forEach(pane => pane.classList.remove('active'));
    const targetPane = document.getElementById(targetPaneId);
    if (targetPane) targetPane.classList.add('active');

    // Trigger Chart.js resize update
...
function initPeriodFilter() {
    ...
    select?.addEventListener('change', (e) => {
        ...
        let statusText = 'Exibindo dados completos (2025/2026)';
        if (val === 'semestre1') statusText = 'Exibindo 1º Semestre (Jan - Jun)';
        else if (val === 'semestre2') statusText = 'Exibindo 2º Semestre (Jul - Dez)';
        else if (val === 'picos') statusText = 'Exibindo Apenas Meses de Pico (Festas & Inverno)';

        if (badge) {
            badge.innerHTML = `<span class="status-indicator"></span> ${statusText}`;
        }

        // Apply filter to demand charts
        updateDemandaPeriodFilter(val);
    });
}
        ...
        <div class="table-card" data-searchable>
            <div class="table-card-header">
                ...
                <div class="table-actions">
                    <button class="btn btn-outline" onclick="copyTableToClipboard('${t.id}')" aria-label="Copiar dados da tabela ${t.title}">
            ...
            <table class="data-table" id="${t.id}">
                <thead>
                    <tr>${t.headers.map(h => `<th scope="col">${h}</th>`).join('')}</tr>
                </thead>
                <tbody>
